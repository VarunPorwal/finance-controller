...
export function CommandPalette() {
  ...
  const items = useMemo(() => {
    ...
    if (/^exc_/i.test(t)) out.push({ label: `Open decision ${t}`, hint: "Decisions", run: () => router.push(`/app1/decisions?open=${t}`) });
    if (/^(mch|evt)_/i.test(t)) out.push({ label: `Find record ${t}`, hint: "Records", run: () => router.push(`/app1/records?q=${t}`) });
    if (/^run_/i.test(t)) out.push({ label: `Open run ${t}`, hint: "Run", run: () => router.push(`/app1/run`) });
    ...
    <AnimatePresence>
      {paletteOpen && (
        <>
          ...
          />
          <motion.div
            className="a1-pop fixed left-1/2 top-[18vh] z-[70] w-[560px] max-w-[calc(100vw-32px)] -translate-x-1/2 overflow-hidden"
            initial={false}
            animate={{ opacity: 1 }}
            role="dialog"
            aria-modal
          >
            <div className="flex items-center gap-2.5 border-b px-4 py-3">
              <Search size={15} strokeWidth={1.6} className="a1-faint" />
              <input
                ref={inputRef}
                className="flex-1 bg-transparent text-[14px] placeholder:text-[var(--a1-ink-3)]"
              ...
              {items.map((it, i) => (
                <li key={it.label} role="option" aria-selected={i === idx} ref={(el) => { if (i === idx) el?.scrollIntoView({ block: "nearest" }); }}>
                  <button
                    ...
                    style={{ background: i === idx ? "var(--a1-surface-3)" : "transparent" }}
                  >
                    {it.icon === "ask" ? (
                      <Sparkles size={13} strokeWidth={1.6} style={{ color: "var(--fc-accent)" }} />
                    ) : (
                      <ArrowRight size={13} strokeWidth={1.6} className="a1-faint" />
                    )}
                    <span className="min-w-0 flex-1">
                      <span className="block text-[13px]">{it.label}</span>
                      <span className="a1-faint block text-[11px]">{it.hint}</span>
                    </span>
                  </button>
                </li>
              ))}
