...
export function SubBar({
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center justify-between gap-x-4 gap-y-2">
      ...
      <Link href="/run" className="fc-btn shrink-0">
        New run
      </Link>
    </div>
  );
...
export function UnmatchedByRun({ runs, currentRunId, index = 0 }: { runs: RunOut[] | undefined; currentRunId: string | undefined; index?: number }) {
  ...
  const results = useQueries({
    queries: recentRuns.map((r) => ({
      queryKey: ["app", "run", r.run_id, "exceptions", "open-subtotal"],
      queryFn: async () => {
      ...
      <div className="flex h-full items-start gap-3">
        ...
        <div className="flex h-full min-w-0 flex-1 flex-col">
          ...
          <div className="mt-3 flex gap-2">
            <Link href="/decisions" className="fc-btn fc-btn--light">
              Review exceptions
            </Link>
...
function NeedsRow({ e, counterparty }: { e: Exception; counterparty: string | undefined }) {
  ...
  return (
    <Link href={`/decisions?open=${encodeURIComponent(e.exception_id)}`} className="block">
  ...
  return (
    ...
    >
      <div className="flex shrink-0 items-center justify-between">
        ...
        <Link href="/decisions" className="fc-link">View all</Link>
      </div>
      {!items ? (
...
function SourceBlock({ label, count, index }: { label: string; count: number | undefined; index: number }) {
  return (
    ...
    >
      <Link href="/records" className="fc-card-hover block" style={{ borderRadius: "var(--fc-r-sm)", padding: "10px 14px" }}>
  ...
  return (
    <div className="fc-card mb-3">
      ...
      <div className="flex items-center">
        ...
        <SourceConnector matched={razorpayBank.matched} gap={razorpayBank.gap} href="/decisions" index={0} />
        <SourceBlock label={SOURCE_LABEL.bank} count={bySource?.bank} index={1} />
        <SourceConnector matched={bankLedger.matched} gap={bankLedger.gap} href="/decisions?category=missing_in_ledger" index={1} />
