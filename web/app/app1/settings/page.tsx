              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    <Mail size={14} className="a1-faint pointer-events-none absolute left-3 top-1/2 -translate-y-1/2" />
                    <input
                      type="email"
                      className="a1-input !pl-9"
                ...
                <AnimatePresence>
                  ...
                  {saved && (
                    <motion.div
                      ...
                      exit={{ opacity: 0 }}
                      className="text-[12.5px] text-[var(--a1-ok)]"
                    >
                      Saved.
                    </motion.div>
                  )}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex flex-col gap-0.5">
                    ...
                    <div className="a1-faint text-[12px]">
                ...
                <AnimatePresence>
                  ...
                  {sent && (
                    <motion.div
                      ...
                      className={
                        sent.sent
                          ? "rounded-lg border border-[var(--a1-ok-line)] bg-[var(--a1-ok-soft)] px-3 py-2.5"
                          : "rounded-lg border border-[var(--a1-warn-line)] bg-[var(--a1-warn-soft)] px-3 py-2.5"
                      }
                    >
                      {sent.sent ? (
                        <>
                          <div className="text-[13px] text-[var(--a1-ok)]">
                            Sent to {sent.recipients.length > 0 ? sent.recipients.join(", ") : plural(0, "recipient")}.
                          </div>
                          <div className="a1-muted mt-1 text-[12.5px]">{sent.headline}</div>
                        </>
                      ) : (
                        <>
                          <div className="text-[13px] text-[var(--a1-warn)]">Not sent{sent.reason ? `: ${sent.reason}` : "."}</div>
                          {sent.headline && <div className="a1-muted mt-1 text-[12.5px]">{sent.headline}</div>}
                        </>
                      )}
                    </motion.div>
                  )}
      ...
      <Reveal delay={0.08}>
        <Card>
          ...
          <div className="flex flex-col gap-5 px-5 pb-5">
            <KeyValue
              rows={[
                ["Ruleset in force", run ? <span className="mono" title={run.ruleset_hash}>{hashShort(run.ruleset_hash, 16)}</span> : <span className="a1-faint">No run loaded</span>],
                ["Current run", run ? <span className="mono" title={run.run_id}>{run.run_id}</span> : <span className="a1-faint">—</span>],
            ...
            <ol className="flex flex-col gap-3">
              {THESIS.map((t, i) => (
                <li key={i} className="flex gap-3">
                  <p className="a1-muted text-[13px] leading-relaxed">
                    <span className="text-[var(--a1-ink)]">{t.slice(0, t.indexOf(".") + 1)}</span>
                    {t.slice(t.indexOf(".") + 1)}
                  </p>
                </li>
              ))}
